...
func MemberToSpiffe(member interface{}, enableAthenzCloudSpiffee bool) ([]string, error) {
	if member == nil {
	...
	return PrincipalToSpiffe(memberStr, enableAthenzCloudSpiffee)
}
...
func RoleToSpiffe(athenzDomainName string, roleName string, enableAthenzCloudSpiffee bool) ([]string, error) {
	...
	namespace := athenz.DomainToNamespace(athenzDomainName)
	principal := fmt.Sprintf("%s.%s", athenzDomainName, roleName)

	spiffeNames := []string{
		fmt.Sprintf("%s/ra/%s", athenzDomainName, roleName),
	}
	if enableAthenzCloudSpiffee {
		newSpiffeNames := []string{
			fmt.Sprintf("athenz.cloud/ns/%s/ra/%s", namespace, principal),
			fmt.Sprintf("athenz.cloud/ns/%s/ra/%s", athenzDomainName, principal),
			fmt.Sprintf("athenz.cloud/ns/default/ra/%s", principal),
		}
...
func PrincipalToSpiffe(principal string, enableAthenzCloudSpiffee bool) ([]string, error) {
	if len(principal) == 0 {
