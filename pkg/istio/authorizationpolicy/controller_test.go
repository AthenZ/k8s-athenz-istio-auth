...
func newFakeController(athenzDomain *adv1.AthenzDomain, service *v1.Service, fake bool, apEnabledList string, stopCh <-chan struct{}, standaloneMode bool) *Controller {
	...
	c.rbacProvider = rbacv2.NewProvider(componentsEnabledAuthzPolicy, c.enableOriginJwtSubject, false, "proxy-principals")
...
func TestNewController(t *testing.T) {
	...
	standAloneMode := true
	c := NewController(configStoreCache, fakeIndexInformer, fakeAthenzInformer, istioClientSet, apResyncInterval, true, &common.ComponentEnabled{}, "proxy-principals", standAloneMode, false)
